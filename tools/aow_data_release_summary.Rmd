...
subtitle: "Recruitment and data collection to end of July 2026"
output:
  html_document:
    toc: true
    toc_float: true
...
repeat_counter <- function(df) {
  ...
  df |>
    mutate(year_group_2022 = case_when(recruitment_era == "2022-23" ~ year_group,
                                       ...
                                       recruitment_era == "2025-26" ~ year_group - 3)) |>
...
Number of participants with completed datasets of each type across Y8-10 in each recruitment year. The plot below reflects data in the module 25 dataset released in 2026. This includes survey responses from all years where questions were carried forward. So, it is a longitudinal dataset containing responses to all questions from module 25 and all responses to matching questions from previous modules in previous years.

```{r survey_plots, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}

srv25 |> yg_plot("AoW surveys")
...
bld |> yg_plot("AoW school visit: blood tests")
...
### Participants received (total) and with data

```{r summary_tbl, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}

summ_df <- denom |> filter(recruitment_era != "2021-22") |>
  summarise(total = n(),
            across(c(has_data, has_survey, has_measure, has_bloods, has_ckcog), ~sum(.))) |>
  ungroup() |>
...
### Participants received (total) and with data, BiB only

```{r summary_tbl_bib, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}

summ_df_bib <- denom |>  filter(recruitment_era != "2021-22") |> filter(is_bib == "BiB") |>
  summarise(total = n(),
            across(c(has_data, has_survey, has_measure, has_bloods, has_ckcog), ~sum(.))) |>
  ungroup() |>
  relocate(has_data, .after = total)
...
### Participants received (unique) and with data

```{r summary_tbl_u, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}

summ_df <- denom |> filter(recruitment_era != "2021-22") |>
  group_by(aow_person_id) |>
  summarise(multi_survey = max(ifelse(sum(has_survey, na.rm = TRUE) > 1 & has_survey == 1, 1, 0)),
         across(c(has_data, has_survey, has_measure, has_bloods, has_ckcog), ~max(.))) |>
  ungroup() |>
  unique() |>
  summarise(total = n(),
            across(c(has_data, has_survey, multi_survey, has_measure, has_bloods, has_ckcog), ~sum(.))) |>
  ungroup() |>
  relocate(has_data, .after = total)
...
### Participants received (unique) and with data, BiB only

```{r summary_tbl_bib_u, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}
...
### Participants received (total) and with data by year group

```{r summary_tbl_yg, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}

summ_df <- denom |>  filter(recruitment_era != "2021-22") |> group_by(aow_person_id) |> arrange(recruitment_date) |>
  mutate(previous_survey = ifelse(cumsum(has_survey) > 1 & has_survey == 1, 1, 0)) |>
  ungroup() |>
  unique() |> group_by(year_group) |>
  summarise(total = n(),
            across(c(has_data, has_survey, previous_survey, has_measure, has_bloods, has_ckcog), ~sum(.))) |>
  ungroup() |>
  mutate(year_group = as.integer(year_group)) |>
  select(year_group, everything()) |>
  arrange(year_group) |>
  relocate(has_data, .after = total)
...
### Participants received (total) and with data by year group, BiB only

```{r summary_tbl_bib_yg, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}

summ_df_bib <- denom |>  filter(recruitment_era != "2021-22") |> filter(is_bib == "BiB") |> 
  group_by(aow_person_id) |> arrange(recruitment_date) |>
  mutate(previous_survey = ifelse(cumsum(has_survey) > 1 & has_survey == 1, 1, 0)) |>
  ungroup() |>
  unique() |>
  group_by(year_group) |>
...
### Participants received (total) and with data by recruitment year

```{r summary_tbl_ry, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}

summ_df <- denom |>  filter(recruitment_era != "2021-22") |> 
  group_by(aow_person_id) |> arrange(recruitment_date) |>
  mutate(previous_survey = ifelse(cumsum(has_survey) > 1 & has_survey == 1, 1, 0)) |>
  ungroup() |>
  unique() |> group_by(recruitment_era) |>
  summarise(total = n(),
            across(c(has_data, has_survey, previous_survey, has_measure, has_bloods, has_ckcog), ~sum(.))) |>
  ungroup() |>
  select(recruitment_era, everything()) |>
  arrange(recruitment_era) |>
  relocate(has_data, .after = total)
...
### Participants received (total) and with data by recruitment year, BiB only

```{r summary_tbl_bib_ry, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}
...
### Participants received (total) and with data by subcohort

```{r summary_tbl_sc, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}

summ_df <- denom |>  filter(recruitment_era != "2021-22") |> group_by(aow_person_id) |> arrange(recruitment_date) |>
  mutate(previous_survey = ifelse(cumsum(has_survey) > 1 & has_survey == 1, 1, 0)) |>
  ungroup() |>
  unique() |> group_by(recruitment_era, year_group) |>
  summarise(total = n(),
            across(c(has_data, has_survey, previous_survey, has_measure, has_bloods, has_ckcog), ~sum(.))) |>
  ungroup() |>
  mutate(year_group = as.integer(year_group)) |>
...
### Participants received (total) and with data by subcohort, BiB only

```{r summary_tbl_bib_sc, echo=FALSE, fig.height=7, fig.width=9, message=FALSE, warning=FALSE}
...
summ_df_bib <- denom |>  filter(recruitment_era != "2021-22") |> filter(is_bib == "BiB") |> 
  ...
  ungroup() |>
  unique() |>
  group_by(recruitment_era, year_group) |>
  summarise(total = n(),
            across(c(has_data, has_survey, previous_survey, has_measure, has_bloods, has_ckcog), ~sum(.))) |>
  ungroup() |>
  mutate(year_group = as.integer(year_group)) |>
