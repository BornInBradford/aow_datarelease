...
new_module <- readRDS("U:/Born In Bradford - Confidential/Data/BiB/processing/AoW/survey/data/aow_survey_module25_linked.rds")

pre_mod24 <- readRDS("U:/Born in Bradford - AOW Raw Data/previous_release/survey/aow_survey_module24_labelled.rds")

# check duplicates in aow_recruitment_id in previous release
pre_mod24$aow_recruitment_id |> duplicated() |> any()

new_names <- names(new_module)
new_names_stems <- new_names |> aow_srv_var_stems()

pre_rec_ids <- c(pre_mod24$aow_recruitment_id) |>
  unique()
...
pre_mod24 <- pre_mod24 |> format_year_group()

# derive previous module merged administrative dataset
pre_mod_admin <- pre_mod24 |> select(any_of(aow_survey_column_order()))
...
pre_mod24 <- pre_mod24 |> select(starts_with(new_names_stems)) |> select(-any_of(aow_pre_mod_drop_cols()))

# merge all previous module data to be carried forward
pre_merged <- pre_mod_admin |>
  left_join(pre_mod24, by = "aow_recruitment_id")
