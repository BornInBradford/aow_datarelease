# Module 25 survey online/offline merge and tidy up

source("tools/aow_survey_functions.R")

redcap_project_name <- c("aow_module_25_online_survey566e", "aow_module_25_offline_survey13f1")

# data
online <- read_dta("U:\\Born in Bradford - AOW Raw Data\\redcap\\surveys\\data\\Survey_Module25_Online.dta")
offline <- read_dta("U:\\Born in Bradford - AOW Raw Data\\redcap\\surveys\\data\\Survey_Module25_Offline.dta")
...
timestamps <- timestamps |> filter(project_name %in% redcap_project_name) |> select(-project_id, -project_name)
...
###### online <- online |> mutate(awb1_1_id = as.character(awb1_1_id))
...
# export
saveRDS(mod_allcols, "U:/Born In Bradford - Confidential/Data/BiB/processing/AoW/survey/data/aow_survey_module25_merged.rds")
