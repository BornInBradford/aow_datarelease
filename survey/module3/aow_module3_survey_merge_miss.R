# Module 3 survey online/offline merge and tidy up

source("tools/aow_survey_functions.R")

redcap_project_name <- "aow_module_3_online_survey"
...
max_online_version <- max(online$mod3_version, na.rm = TRUE)
min_offline_version <- min(offline$mod3_version, na.rm = TRUE)
max_offline_version <- max(offline$mod3_version, na.rm = TRUE)

# data dictionary
online_dict <- read_csv("survey/redcap/AoWModule3OnlineSurvey_DataDictionary_2023-06-15.csv",
...
online$year_group <- as.numeric(online$year_group)
...
mod_allcols <- mod_allcols |> mutate(awb5_1_food_dt_6_r7 = case_when(survey_mode == 1 ~ coalesce(awb5_1_food_dt_6_r7, awb5_1_food_dt_6),
                                                                     survey_mode == 2 ~ coalesce(awb5_1_food_dt_6_r7, awb5_1_food_dt_6_a6)))
...
# export
saveRDS(mod_allcols, "U:/Born In Bradford - Confidential/Data/BiB/processing/AoW/survey/data/aow_survey_module3_merged.rds")
