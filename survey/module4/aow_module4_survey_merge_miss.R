# Module 4 survey online/offline merge and tidy up
...
min_online_version <- min(online$mod4_version, na.rm = TRUE)
max_online_version <- max(online$mod4_version, na.rm = TRUE)
min_offline_version <- min(offline$mod4_version, na.rm = TRUE)
max_offline_version <- max(offline$mod4_version, na.rm = TRUE)

# data dictionary
online_dict <- read_csv("survey/redcap/AoWModule4OnlineSurvey_DataDictionary_2023-06-15.csv",
...
for(c in 1:nrow(vlabel_conflict)) {
  print(vlabel_conflict$variable[c])
  print(vlabel_conflict$on_categories[c])
  print(vlabel_conflict$off_categories[c])
}

# There are non-trivial differences in the response categories for:
# awb7_1_bullying_1
# awb7_1_recommend_1
# awb6_7_opp_dig_sklls_r8___6 (can isolate the category as it's a checkbox response)
#
# create online vs offline versions of these by renaming

for(v in c("awb7_1_bullying_1", "awb7_1_recommend_1")) {
  
  online <- online |> aow_suffix_var(v, "on")
  online_dict <- online_dict |> aow_suffix_var_dict(v, "on")
  
  offline <- offline |> aow_suffix_var(v, "off")
  offline_dict <- offline_dict |> aow_suffix_var_dict(v, "off")
  
}

# the checkbox response variables are named differently in the 
...
online$awb8_2_problem_3 <- as.character(online$awb8_2_problem_3)
online$awb8_2_problem_4 <- as.character(online$awb8_2_problem_4)
offline$awb6_1_neg_exp_othr_r5 <- as.character(offline$awb6_1_neg_exp_othr_r5)
...
mod_allcols <- online |> bind_rows(offline) |>
  ...
  rename(survey_version = mod4_version)
...
# export
saveRDS(mod_allcols, "U:/Born In Bradford - Confidential/Data/BiB/processing/AoW/survey/data/aow_survey_module4_merged.rds")
